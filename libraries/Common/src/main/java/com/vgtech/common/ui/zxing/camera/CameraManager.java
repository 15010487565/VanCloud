...
package com.vgtech.common.ui.zxing.camera;
...
public final class CameraManager {
    ...
    public CameraManager(Context context) {
        ...
        int spit = CommonUtils.convertDipOrPx(context, 120);
        MIN_FRAME_WIDTH = dm.widthPixels - spit;
        MIN_FRAME_HEIGHT = dm.widthPixels - spit;
        MAX_FRAME_WIDTH = dm.widthPixels - spit;
        MAX_FRAME_HEIGHT = dm.widthPixels - spit;
...
//			int topOffset = (screenResolution.y - height) * 2 / 4;//5
            if (macher())
                framingRect = new Rect(leftOffset, topOffset, leftOffset + width, height + topOffset);//控制扫描窗体宽高
            else
                framingRect = new Rect(leftOffset, topOffset, leftOffset + width, height + topOffset);//控制扫描窗体宽高
    ...
    private boolean macher() {
        String mtype = "";
        if (android.os.Build.MODEL.equals("2014813"))
            mtype = "HM 2";
        Pattern pattern = Pattern.compile("^HM 2.*");//查找以MI 2开头,任意结尾的字符串
