...
public class TitleIndicator extends LinearLayout implements View.OnClickListener,
    ...
    public TitleIndicator(Context context, AttributeSet attrs) {
        ...
        // Retrieve the colors to be used for this com.vgtech.personaledition.view and apply them.
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mTotal != 0) {
            int width = getWidth();
            mPerItemWidth = width / mTotal;
            if (mItemWidth != 0) {
                width = mPerItemWidth * (mTotal - 1);
            }
            int tabID = mSelectedTab;
            scroll_x = (mCurrentScroll - ((tabID) * (width + mViewPager.getPageMargin()))) / mTotal;
        } else {
        ...
        path.rewind();
        float offset = 0;
        float left = mSelectedTab * mPerItemWidth + offset + scroll_x + convertDipOrPx(mContext, 10);
        float right = (mSelectedTab + 1) * mPerItemWidth - offset + scroll_x - convertDipOrPx(mContext, 10);

        float midile = (right + left) / 2;
        float left_x = midile - convertDipOrPx(mContext, 20);
        float right_x = midile + convertDipOrPx(mContext, 20);
    ...
    public void init(int startPos, List<TabInfo> tabs, ViewPager mViewPager, int tabCount) {
        mItemWidth = getContext().getResources().getDisplayMetrics().widthPixels / tabCount;
        init(startPos, tabs, mViewPager);
    }
    ...
    public void updateTitle(int postion, String title) {
        View child = getChildAt(postion);
        TextView titleTv = (TextView) child.findViewById(R.id.tab_title);
