 ...
 */
public class OrganizationWorkGroupFragment extends Fragment implements Animation.AnimationListener, AdapterView.OnItemClickListener, View.OnClickListener, OrganizationAdapterLister {
        ...
        mCategoryWait.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (TextUtils.isEmpty(level)) {
                    mCategoryAdapter.addAllData(getOrganizations(0, null));
                } else {
    ...
    private void initListData() {
        ...
        new AsyncTask<Void, Void, List<Node>>() {
            ...
            protected List<Node> doInBackground(Void... params) {
                List<User> userList = TenantPresenter.isVanTop(getActivity()) ? WorkRelation.queryVantopWorkGroup(getActivity()) : WorkRelation.queryWorkGroup(getActivity());
                ArrayList<WorkGroup> workGroups = WorkGroup.queryWorkGroup(getActivity());
    ...
    public List<Organization> getOrganizations(int level, String pid) {
        ...
        if (selectedListener != null && selectedListener.getSelectMode() == OrganizationSelectedListener.SELECT_MULTI) {
            Organization all = new Organization("", "all", TextUtils.isEmpty(pid) ? "workGroup" : pid);
            if (!nodes.isEmpty())
                departs.add(all);
        }
        for (Node node : nodes) {
            if (level == 0 && node.getLevel() == level || node.getLevel() == level && (!TextUtils.isEmpty(pid) && pid.equals(node.getpId()) || TextUtils.isEmpty(pid) && TextUtils.isEmpty(node.getpId()))) {
                if (!node.isUser()) {
    ...
    private void setCatTitle(Organization organization) {
        if (mLuncherLayout.getChildCount() == 0) {
            ...
            lableTv.setTag(0);
            lableTv.setText(R.string.tab_work);
