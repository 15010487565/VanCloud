 ...
 */
public class ApproverAdapter extends DataAdapter<Approver> {


    Context context;

    public ApproverAdapter(Context context, List dataSource) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final Approver al = dataSource.get(position);
        if (!TextUtils.isEmpty(al.staff_name) && !"null".equals(al.staff_name))
            viewHolder.approvalNameTxt.setText(al.staff_name);

        String status = al.status;

        if ("0".equals(status)) {
            viewHolder.statusRemark.setText(context.getResources().getString(R.string.vantop_approving));
            viewHolder.statusRemark.setTextColor(context.getResources().getColor(R.color.approvaling_txt));
            ...
            viewHolder.approvalDate.setText(al.approved_date);
